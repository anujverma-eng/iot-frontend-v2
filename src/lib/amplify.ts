...
Hub.listen('pubsub', (data: any) => {
  const { payload } = data;
  console.log('[MQTT Hub]', payload.event, payload.data);
  
  if (payload.event === 'connected') {
    console.log('✅ [MQTT] Connected to IoT Core');
  } else if (payload.event === 'disconnected') {
    console.log('❌ [MQTT] Disconnected from IoT Core');
  } else if (payload.event === 'connection_failed') {
    console.error('🚫 [MQTT] Connection failed:', payload.data);
  }
});
