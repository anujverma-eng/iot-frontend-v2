  ...
  return (
    ...
    >
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {!item.acknowledged && <div className="w-2 h-2 rounded-full bg-primary flex-shrink-0" />}
                      </div>
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-default-900">Manage Notifications</h2>
        </div>
        <Button color="primary" startContent={<Icon icon="lucide:plus" className="w-3 h-3" />} onPress={handleAddAlert}>
      ...
      <div className="space-y-4">
        {loading ? (
          ...
          <>
            <div className="overflow-x-auto border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm relative">
              {loading && (
              ...
              <table className="w-full min-w-full table-auto border-collapse">
                <thead>
                  <tr className="bg-gray-100 dark:bg-gray-100/20">
                    <th className="px-4 py-3 text-left text-sm font-medium text-dark">EVENT</th>
                    <th className="px-4 py-3 text-left text-sm font-medium text-dark">DEVICE</th>
                    <th className="px-4 py-3 text-left text-sm font-medium text-dark">ACTION</th>
                    <th className="px-4 py-3 text-left text-sm font-medium text-dark">RECIPIENT(S)</th>
                    <th className="px-4 py-3 text-left text-sm font-medium text-dark">ON/OFF</th>
                      ...
                      <td className="px-4 py-3 text-sm">
                        <div className="flex flex-col gap-1">
                          {alert.channels.email?.addresses?.map((email, idx) => (
                            <p key={`email-${idx}`} className="text-sm text-default-700">
                              {email}
                            </p>
                          ))}
                          {alert.channels.sms?.phoneNumbers?.map((phone, idx) => (
                            <p key={`phone-${idx}`} className="text-sm text-default-700">
                              {phone}
                            </p>
                          ))}
                      ...
                      <td className="px-4 py-3 text-sm text-center">
                        <div className="flex items-center justify-center gap-2">
                          <Tooltip content="Edit">
                            <Button isIconOnly size="sm" variant="light" onPress={() => handleEditAlert(alert)}>
          ...
          <div className="flex flex-col items-center justify-center h-64 gap-4">
            <Icon icon="lucide:bell-off" className="w-12 h-12 text-default-400" />
            <p className="text-default-400">No alerts configured yet</p>
            <Button color="primary" variant="flat" size="sm" onPress={handleAddAlert}>
      ...
      <Modal isOpen={isNotificationsModalOpen} onClose={onNotificationsModalClose} size="5xl" scrollBehavior="inside">
                ...
                {historyPagination.total > 0 && (
                  <div className="flex justify-between items-center p-4 border-t border-default-200">
                    <p className="text-sm text-default-500">
                      Showing{" "}
                      {Math.min((historyPagination.page - 1) * historyPagination.limit + 1, historyPagination.total)} to{" "}
                      {Math.min(historyPagination.page * historyPagination.limit, historyPagination.total)} of{" "}
                      {historyPagination.total} notifications
                    </p>
                    <Pagination
